...
@php
    $buttonClasses = generateClasses([
        'flex font-medium items-center h-7 items-center border-b-2 relative top-[7px] space-x-2 pb-[20px] transition focus:outline-none',
...
@elseif ($tag === 'a')
    <a {{ $attributes->class([$buttonClasses]) }}>
        @if ($icon)
            <x-dynamic-component :component="$icon" class="w-4 h-4 opacity-80" />
        @endif
        <span @if($active) class="block rounded-lg border-gray-200/70 py-1" @endif>{{ $slot }}</span>
    </a>
@endif
